 ...
 *
 * Lists user's custom monster moods with options to create, edit, and delete.
...
export function CustomMoodsCard({ local }: CustomMoodsCardProps): ReactElement {
  ...
  const [editingItem, setEditingItem] = useState<MoodDetail | null>(null)
  const [saving, setSaving] = useState(false)
  const [dialogKey, setDialogKey] = useState(0)

  /** Sort items alphabetically by name */
  const sortItems = useCallback(
    (list: MoodDetail[]): MoodDetail[] =>
      [...list].sort((a, b) => a.mood_name.localeCompare(b.mood_name)),
    []
  )
  ...
  return (
    <Card className="p-0 border gap-0">
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-center p-8 text-center">
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                No custom moods stir in the shadows yet.
              </p>
              <p className="text-xs text-muted-foreground">
                Create a custom mood to see it appear here.
              </p>
            </div>
          </div>
        ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell>
                      <div className="font-medium">{item.mood_name}</div>
                    </TableCell>
      ...
      <CustomItemDialog
        ...
        onSave={handleEdit}
        saving={saving}
        initialName={editingItem?.mood_name}
