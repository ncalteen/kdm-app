...
create policy "Allow all for owner" on hunt for all using (
  auth.uid() = (
    select user_id
    from settlement
...
create policy "Allow all for shared users" on hunt for all using (
  exists (
    select 1
    from settlement s
...
-- Junction table: links hunts to hunt monster data
...
create policy "Allow all for owner" on hunt_hunt_monster_data for all using (
  auth.uid() = (
    select user_id
    from settlement
    where id = (
...
create policy "Allow all for shared users" on hunt_hunt_monster_data for all using (
  exists (
    select 1
    from settlement s
    where s.id = (
...
-- Junction table: links hunts to hunt survivor data
...
create policy "Allow all for owner" on hunt_hunt_survivor_data for all using (
  auth.uid() = (
    select user_id
    from settlement
    where id = (
...
create policy "Allow all for shared users" on hunt_hunt_survivor_data for all using (
  exists (
    select 1
    from settlement s
    where s.id = (
