...
create policy "Allow all for owner" on showdown_survivor_data for all using (
  auth.uid() = (
    select user_id
    from settlement
...
create policy "Allow all for shared users" on showdown_survivor_data for all using (
  exists (
    select 1
    from settlement s
