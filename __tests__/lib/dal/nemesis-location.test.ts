...
describe('getNemesisLocationIds', () => {
  it('throws when nemesisId is null', async () => {
    await expect(getNemesisLocationIds(null)).rejects.toThrow('Required: Nemesis ID')
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })

  it('throws when nemesisId is undefined', async () => {
    await expect(getNemesisLocationIds(undefined)).rejects.toThrow('Required: Nemesis ID')
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })
  ...
  it('throws when data is null', async () => {
    ...
    await expect(getNemesisLocationIds('n1')).rejects.toThrow('Nemesis Location ID(s) Not Found')
  })
})

describe('getNemesisLocations', () => {
  it('throws when nemesisId is null', async () => {
    await expect(getNemesisLocations(null)).rejects.toThrow('Required: Nemesis ID')
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })

  it('throws when nemesisId is undefined', async () => {
    await expect(getNemesisLocations(undefined)).rejects.toThrow('Required: Nemesis ID')
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })
  ...
  it('throws when data is null', async () => {
    ...
    await expect(getNemesisLocations('n1')).rejects.toThrow('Nemesis Location Data Not Found')
  })
})

describe('addNemesisLocation', () => {
  it('inserts a nemesis location and returns its id', async () => {
    const mockSingle = vi.fn().mockResolvedValue({ data: { id: 'nll1' }, error: null })
    const mockSelect = vi.fn().mockReturnValue({ single: mockSingle })
    const mockInsert = vi.fn().mockReturnValue({ select: mockSelect })
    mockSupabase.from.mockReturnValue({ insert: mockInsert })

    const result = await addNemesisLocation({ nemesis_id: 'n1', location_id: 'loc1' })

    expect(result).toBe('nll1')
    expect(mockSupabase.from).toHaveBeenCalledWith('nemesis_location')
    expect(mockInsert).toHaveBeenCalledWith({ nemesis_id: 'n1', location_id: 'loc1' })
  })
  ...
  it('throws when insert fails', async () => {
    ...
    await expect(addNemesisLocation({ nemesis_id: 'n1', location_id: 'loc1' })).rejects.toThrow(
      'Error Adding Nemesis Location: Insert failed'
    )
  })
})
...
describe('updateNemesisLocation', () => {
  ...
  it('throws when update fails', async () => {
    ...
    await expect(updateNemesisLocation('nll1', { location_id: 'loc2' })).rejects.toThrow(
      'Error Updating Nemesis Location: Update failed'
    )
  })
})
