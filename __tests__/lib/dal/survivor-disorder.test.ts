...
describe('getSurvivorDisorders', () => {
  it('throws when survivorId is null', async () => {
    await expect(getSurvivorDisorders(null)).rejects.toThrow(
  ...
  it('throws when survivorId is undefined', async () => {
    await expect(getSurvivorDisorders(undefined)).rejects.toThrow(
      'Required: Survivor ID'
    )
  })
...
describe('addSurvivorDisorder', () => {
  it('inserts and returns the junction row id', async () => {
    const mockSingle = vi
      .fn()
      .mockResolvedValue({ data: { id: 'sd-new' }, error: null })
  ...
  it('throws when insert fails', async () => {
    ...
    await expect(
      addSurvivorDisorder('survivor-1', 'disorder-1')
    ).rejects.toThrow('Error Adding Survivor Disorder: Insert failed')
  })
})
...
describe('removeSurvivorDisorder', () => {
  it('deletes matching junction row', async () => {
    ...
    await expect(
      removeSurvivorDisorder('survivor-1', 'disorder-1')
  ...
  it('throws when delete fails', async () => {
    ...
    await expect(
      removeSurvivorDisorder('survivor-1', 'disorder-1')
    ).rejects.toThrow('Error Removing Survivor Disorder: Delete failed')
  })
})
...
describe('updateSurvivorDisorder', () => {
  ...
  it('throws when update fails', async () => {
    ...
    await expect(
      updateSurvivorDisorder('sd-1', { disorder_id: 'disorder-2' })
    ).rejects.toThrow('Error Updating Survivor Disorder: Update failed')
  })
})
