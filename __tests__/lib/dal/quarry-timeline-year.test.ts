...
describe('getQuarryTimelineYears', () => {
  it('throws when quarryId is null', async () => {
    await expect(getQuarryTimelineYears(null)).rejects.toThrow('Required: Quarry ID')
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })

  it('throws when quarryId is undefined', async () => {
    await expect(getQuarryTimelineYears(undefined)).rejects.toThrow('Required: Quarry ID')
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })
  ...
  it('returns timeline years with campaignType filter', async () => {
    ...
    const result = await getQuarryTimelineYears('q1', CampaignType.PEOPLE_OF_THE_LANTERN)
  ...
  it('throws when data is null', async () => {
    ...
    await expect(getQuarryTimelineYears('q1')).rejects.toThrow('Quarry Timeline Year(s) Not Found')
  })
})
...
describe('addQuarryTimelineYear', () => {
  ...
  it('throws when insert fails', async () => {
    ...
    await expect(
      addQuarryTimelineYear({ quarry_id: 'q1', year_number: 1, entries: [], campaign_types: [] })
...
describe('updateQuarryTimelineYear', () => {
  it('updates a quarry timeline year successfully', async () => {
    ...
    await expect(updateQuarryTimelineYear('ty1', { year_number: 2 })).resolves.toBeUndefined()
  ...
  it('throws when update fails', async () => {
    ...
    await expect(updateQuarryTimelineYear('ty1', { year_number: 2 })).rejects.toThrow(
      'Error Updating Quarry Timeline Year: Update failed'
    )
  })
})
