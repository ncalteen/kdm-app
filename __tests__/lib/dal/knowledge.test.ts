...
const { getKnowledges, addKnowledge, updateKnowledge, removeKnowledge } =
...
describe('getKnowledges', () => {
  ...
  const nonCustomKnowledge = {
    id: 'k1',
    custom: false,
    knowledge_name: 'Horology',
    philosophy_id: 'p1'
  }
  const userCustomKnowledge = {
    id: 'k2',
    custom: true,
  ...
  it('returns every knowledge surfaced by RLS', async () => {
    ...
    mockSupabase.from.mockReturnValueOnce({
      select: vi.fn().mockResolvedValue({
        data: [nonCustomKnowledge, userCustomKnowledge, sharedKnowledge],
        error: null
      })
    })
  ...
  it('throws when auth errors', async () => {
    ...
    await expect(getKnowledges()).rejects.toThrow(
      'Error Fetching User: Auth error'
    )
  })

  it('throws when the knowledge query fails', async () => {
    mockSupabase.auth.getUser.mockResolvedValue({
      data: { user: mockUser },
      error: null
    })
    ...
    await expect(getKnowledges()).rejects.toThrow(
      'Error Fetching Knowledges: DB error'
    )
  })
...
describe('addKnowledge', () => {
  ...
  it('inserts a non-custom knowledge without user_id', async () => {
    ...
    const mockSingle = vi
      .fn()
      .mockResolvedValue({ data: mockKnowledge, error: null })
  ...
  it('inserts a custom knowledge with user_id', async () => {
    ...
    const mockSingle = vi
      .fn()
      .mockResolvedValue({ data: customKnowledge, error: null })
    const mockSelect = vi.fn().mockReturnValue({ single: mockSingle })
    const mockInsert = vi.fn().mockReturnValue({ select: mockSelect })
    mockSupabase.from.mockReturnValue({ insert: mockInsert })
...
describe('updateKnowledge', () => {
  ...
  it('throws when update fails', async () => {
    ...
    await expect(
      updateKnowledge('k1', { knowledge_name: 'Horology' })
    ).rejects.toThrow('Error Updating Knowledge: Update failed')
  })
})
...
describe('removeKnowledge', () => {
  ...
  it('throws when delete fails', async () => {
    ...
    await expect(removeKnowledge('k1')).rejects.toThrow(
      'Error Removing Knowledge: Delete failed'
    )
  })
})
