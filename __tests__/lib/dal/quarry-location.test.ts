...
describe('getQuarryLocationIds', () => {
  it('throws when quarryId is null', async () => {
    await expect(getQuarryLocationIds(null)).rejects.toThrow(
      'Required: Quarry ID'
    )
  ...
  it('throws when quarryId is undefined', async () => {
    await expect(getQuarryLocationIds(undefined)).rejects.toThrow(
      'Required: Quarry ID'
    )
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })
  ...
  it('throws when data is null', async () => {
    ...
    await expect(getQuarryLocationIds('q1')).rejects.toThrow(
      'Quarry Location ID(s) Not Found'
    )
  })
})
...
describe('getQuarryLocations', () => {
  it('throws when quarryId is null', async () => {
    await expect(getQuarryLocations(null)).rejects.toThrow(
      'Required: Quarry ID'
    )
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })
  ...
  it('throws when quarryId is undefined', async () => {
    await expect(getQuarryLocations(undefined)).rejects.toThrow(
      'Required: Quarry ID'
    )
    expect(mockSupabase.from).not.toHaveBeenCalled()
  })
  ...
  it('throws when data is null', async () => {
    ...
    await expect(getQuarryLocations('q1')).rejects.toThrow(
      'Quarry Location Data Not Found'
    )
  })
})
...
describe('addQuarryLocation', () => {
  it('inserts a quarry location junction and returns its id', async () => {
    ...
    const result = await addQuarryLocation({
      quarry_id: 'q1',
      location_id: 'loc1'
    })
  ...
  it('throws when insert fails', async () => {
    ...
    await expect(
      addQuarryLocation({ quarry_id: 'q1', location_id: 'loc1' })
    ).rejects.toThrow('Error Adding Quarry Location: Insert failed')
  })
})
...
describe('updateQuarryLocation', () => {
  it('updates a quarry location successfully', async () => {
    ...
    await expect(
      updateQuarryLocation('ql1', { location_id: 'loc2' })
  ...
  it('throws when update fails', async () => {
    ...
    await expect(
      updateQuarryLocation('ql1', { location_id: 'loc2' })
    ).rejects.toThrow('Error Updating Quarry Location: Update failed')
  })
})
