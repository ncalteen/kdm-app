...
describe('getSurvivorFightingArts', () => {
  it('throws when survivorId is null', async () => {
    await expect(getSurvivorFightingArts(null)).rejects.toThrow(
  ...
  it('throws when survivorId is undefined', async () => {
    await expect(getSurvivorFightingArts(undefined)).rejects.toThrow(
      'Required: Survivor ID'
    )
  })
...
describe('addSurvivorFightingArt', () => {
  it('inserts and returns the junction row id', async () => {
    const mockSingle = vi
      .fn()
      .mockResolvedValue({ data: { id: 'sfa-new' }, error: null })
...
describe('removeSurvivorFightingArt', () => {
  it('deletes matching junction row', async () => {
    ...
    await expect(
      removeSurvivorFightingArt('survivor-1', 'fa-1')
  ...
  it('throws when delete fails', async () => {
    ...
    await expect(
      removeSurvivorFightingArt('survivor-1', 'fa-1')
    ).rejects.toThrow('Error Removing Survivor Fighting Art: Delete failed')
  })
})
...
describe('updateSurvivorFightingArt', () => {
  ...
  it('throws when update fails', async () => {
    ...
    await expect(
      updateSurvivorFightingArt('sfa-1', { fighting_art_id: 'fa-2' })
    ).rejects.toThrow('Error Updating Survivor Fighting Art: Update failed')
  })
})
