 ...
 * new survivor.
 */
export const NewSurvivorInputSchema = z
  .object({
     ...
     * Parent 1 ID
     *
     * Optional reference to another survivor in the same settlement
     * representing the new survivor's first parent.
     */
    parent1Id: z
      .string()
      .min(1, 'A nameless parent cannot be recorded.')
      .optional(),
    /**
     * Parent 2 ID
     *
     * Optional reference to another survivor in the same settlement
     * representing the new survivor's second parent. Must differ from
     * `parent1Id` when both are supplied.
     */
    parent2Id: z
      .string()
      .min(1, 'A nameless parent cannot be recorded.')
  ...
  .refine(
    (data) =>
      !data.parent1Id || !data.parent2Id || data.parent1Id !== data.parent2Id,
    {
      message: 'A survivor cannot have the same parent twice.',
      path: ['parent2Id']
    }
  )
